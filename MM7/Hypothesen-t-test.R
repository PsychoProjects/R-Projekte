...
analysis_data_ue <- data %>% select(erlebnisbasiert, i1_ue)
analysis_data_dd <- data %>% select(erlebnisbasiert, i1_dd)

data_analysis <- function(analysis_data){
  # Perform a T-Test
...
data_analysis(analysis_data_ue)
data_analysis(analysis_data_dd)
